...
1. **CMS Site** (Port 8080) - Admin interface for content management
2. **Main Site** (Port 80) - Public-facing SSG site generated from CMS data
...
│  │  - localStorage       │        │  - Served by Express     │ │
...
3. **Serve Production Site (port 80)**
   ```bash
   node server.js
   ```
...
This will:
1. Build the main site from current CMS data
2. Start Express server on port 80
...
### Port conflicts
- CMS on 8080: Change in `gatsby develop` command
- Main site on 80: Change in `server.js` PORT variable
