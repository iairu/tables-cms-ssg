...
const DescendantsTree = ({ cat, allCats, onClose }) => {
  const findChildren = (parentCat) => {
    if (!parentCat) {
      return [];
    }
    const children = allCats.filter(
      (c) => c.sire === parentCat.fullName || c.dam === parentCat.fullName
    );
    return children;
  };

  const renderDescendant = (cat, level) => {
    if (!cat) {
      return null;
    }

    const children = findChildren(cat);
      ...
      <div style={{ paddingLeft: '20px', borderLeft: '1px solid #ccc', position: 'relative' }}>
        ...
        {level > 0 && children.length > 0 && (
          <div style={{ marginLeft: '20px', paddingTop: '10px' }}>
            {children.map((child) => renderDescendant(child, level - 1))}
          </div>
        )}
      </div>
    );
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <h2 style={{ marginTop: 0 }}>Descendants of {cat.fullName}</h2>
